...
- 使用 performance_schema 存储引擎
- 记录在内存中，不会进行持久化
- 主从架构下，performance_schema 不会同步，相互独立
- 使用 server 源代码中的“检测点”来实现事件数据的收集，“事件”就是server内部活动中所做的任何事情以及对应的时间消耗

## 入门

​		在mysql的5.7版本中，性能模式是默认开启的，如果想要显式的关闭的话需要修改配置文件，不能直接进行修改，会报错Variable 'performance_schema' is a read only variable，需要通过配置文件进行修改。
...
注意：在performance_schema库中还包含了很多其他的库和表，能对数据库的性能做完整的监控，需要参考官网详细了解。
...
--12-3、查看每个阶段的锁等待情况
SELECT event_id,event_name,source,timer_wait,object_name,index_name,operation,nesting_event_id FROM events_waits_histoRE nesting_event_id = 1553;
```
